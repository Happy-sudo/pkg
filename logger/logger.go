...
func getLogWriter(name string) zapcore.WriteSyncer {
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fmt.Sprintf("../cmd/log/%s.log", name),
...
func NewZapLogger(name string, encoder zapcore.EncoderConfig, level zap.AtomicLevel, opts ...zap.Option) *ZapLogger {
	//日志切割
	...
	for i := 0; i < len(keyvals); i += 2 {
		if keyvals[i] == "args" {
			if str, ok := keyvals[i+1].(string); ok {
				keyvals[i+1] = str
				//spacelast := str[strings.LastIndex(str[:], " ")+1:]
				//if len(spacelast) > 0 {
				//	fmt.Println(spacelast)
				//	if spacelast[:8] == "password" {
				//		spacebef := str[:strings.LastIndex(str[:], " ")+1]
				//		keyvals[i+1] = spacebef + spacelast[:10] + "****\""
				//	}
				//}
				//keyvals[i+1] = spacelast
			}
		}
